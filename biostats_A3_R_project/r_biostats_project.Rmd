...
*1 group, the population of patients admitted to hospital with SARS-CoV-2 in Mumble, Wisteria.*
...
library(tidyverse)
library(gridExtra)
data = load("datafor16813104.Rdata", )
ls()
```
...
First thing we should do is investigate the distribution of the continuous variables as well as see whether the variance is equal between the comorbidity categories.

To investigate whether the log_los was dependent on patient BMI when accounting for comorbidities, we undertook a multivariate regression analysis.This study consistsed of 500 participants admitted to hospital with SARS-CoV-2 in Mumble, Wisteria.
...
out_ind <- which(mumble$log_los %in% c(out))
out_ind

log_los_outliers = mumble[out_ind,]
```

```{r BMI-outliers}
out <- boxplot.stats(mumble$BMI)$out
out_ind <- which(mumble$BMI %in% c(out))

BMI_outliers = mumble[out_ind,]
BMI_outliers
```
...
#grid.arrange(scatter_BMI_log_los_nil, scatter_BMI_log_los_1_2, scatter_BMI_log_los_3)
```
...
```{r}
ggplot(mumble, aes(y = log_los, x = BMI, col = comorb))+
  geom_point() +
